...
const shader_struct_1 = require("../structs/shader-struct");
...
function parseShaders(buffer, lump) {
    ...
    const num = lump.length / shader_struct_1.ShaderStruct.LENGTH;
    const shaders = [];
    const struct = new shader_struct_1.ShaderStruct(buffer);
    for (let i = 0; i < num; i++) {
